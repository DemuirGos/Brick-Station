...
impl Addressing_data {
    ...
    pub fn handle(&self, cpu_ref: &mut Cpu) -> bool {
        match self {
            ...
            Address_Mode::Imm => {
                cpu_ref.registers.address_abs = cpu_ref.registers.pc;
                cpu_ref.registers.pc += 1;
                false
            },
            Address_Mode::Zp0 => {
                self.address_abs = cpu_ref.read(cpu_ref.registers.pc) as u16;
                cpu_ref.registers.pc += 1;
                self.address_abs &= 0x00FF;
                false
            },
            Address_Mode::Zpx => {
                self.address_abs = cpu_ref.read(cpu_ref.registers.pc) + cpu_ref.registers.x as u16;
                cpu_ref.registers.pc += 1;
                self.address_abs &= 0x00FF;
                false
            },
            Address_Mode::Zpy => {
                self.address_abs = cpu_ref.read(cpu_ref.registers.pc) + cpu_ref.registers.y as u16;
                cpu_ref.registers.pc += 1;
                self.address_abs &= 0x00FF;
                false
            },
            ...
            Address_Mode::Abx => {
                ...
                let hi = cpu_ref.read(cpu_ref.registers.pc) as u16;
                cpu_ref.registers.pc += 1;
                self.address_abs = cpu_ref.registers.x + ((hi << 8) | lo);
            ...
            Address_Mode::Aby => {
                ...
                let hi = cpu_ref.read(cpu_ref.registers.pc) as u16;
                cpu_ref.registers.pc += 1;
                self.address_abs = cpu_ref.registers.y + ((hi << 8) | lo);
                if self.address_abs & 0xFF00 != hi << 8 {
                    true
                }
                else {
            ...
            Address_Mode::Ind => {
                ...
                self.address_abs = (cpu_ref.read(hi_ptr) as u16) << 8 | cpu_ref.read(ptr) as u16;
                false
            },
            Address_Mode::Izx => {
                ...
                let hi = cpu_ref.read((t + cpu_ref.registers.x as u16 + 1) & 0x00FF) as u16;
                self.address_abs = (hi << 8) | lo;
                false
            },
            Address_Mode::Izy => {
            ...
            Address_Mode::Rel => {
                self.address_rel = cpu_ref.read(cpu_ref.registers.pc) as u16;
                cpu_ref.registers.pc += 1;
                if self.address_rel & 0x80 != 0 {
